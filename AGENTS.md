...
- `runtime/service.rs` — one managed service = one listener + accept loop + connection registry. Bind/constructor failures return errors (→ `apply_failed` health), never panics. Connections are independent tasks; `stop(drop_conns)` optionally aborts them (manual restart / TLS rotation only). A per-service connection cap (payload `limit.max_conns`) rejects excess accepts at once.
- `runtime/net.rs` — per-connection forwarding: realm_lb target pick, dial with connect timeout (default 5s), counting bidi copy — splice path for plain/plain legs, `StatStream` userland copy whenever a TLS leg is involved (realm itself goes userland under transport). Both paths pace through `ratelimit::Pacer` when the service carries rate limits.
...
- Quota hard-stopped rules drop out via `applyRuleQuotas` (shared gate); the rule's `limit` JSON is rendered into the payload's enforceable subset (`traffic.service_in/out`, `traffic.conn_in/out`, `connection.service_limit` → `service.limit`) — the agent enforces all three. Rate units in the admin UI are BITS per second (Kbps/Mbps/Gbps, cloud-vendor convention); the payload and ledger stay in bytes/s (UI divides by 8). The `request.*` variant has been REMOVED from the `LimiterConfig` type/schema (no DB migration — `limit` is schemaless JSON TEXT; stale `request` keys in old rows are inert). Per-IP variants remain typed but are not enforced (dropped by the renderer). Quota OBJECTS still never enter the payload (hard-stop stays config-removal based).
